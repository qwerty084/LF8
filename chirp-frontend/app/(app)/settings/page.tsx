  ...
  return (
    <div id="body" className="flex flex-row flex-grow">
      <div className="flex flex-col w-24 h-full shadow-custom">
        <div className="flex items-end justify-center h-full mb-4"></div>
      </div>
      <div className="flex flex-col justify-items-start w-1/6">
        ...
        {topics.map((item, index) => (
          <div
            key={index}
            className={`flex flex-row justify-between px-2 py-1 gap-2 mt-4 ml-4 cursor-pointer rounded-md transition duration-300 hover:shadow-md hover:rounded-r-none ${
              selectedSetting === item.key ? "shadow-md" : ""
            }`}
          ...
          >
            ...
            <p
              className={`mr-4  ${
                selectedSetting === item.key ? "" : "hidden"
              }`}
            >
              {"-->"}
            </p>
