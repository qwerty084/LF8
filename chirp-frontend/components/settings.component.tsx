    ...
    return (
      <div>
        ...
        <div id="faq" className="shadow-custom mb-8 w-[50%] p-2 rounded-md">
          ...
          <span>get more informations about Chirp </span>
          <a
            href="https://github.com/qwerty084/LF8"
        ...
        <div id="problems" className="shadow-custom w-[50%] p-2 rounded-md">
          <p className="text-xl mb-2">
            Report a problem directly into our GitHub Repository
          </p>
          <HelpSupportFunc />
        </div>
      </div>
...
export function AccountFunc() {
  ...
  const [status, setStatus] = useState<string | undefined>(
    session.user.data?.status || undefined,
  );
  const [bio, setBio] = useState<string | undefined>(
    session.user.data?.bio || undefined,
  );
  ...
  async function uploadAvatar() {
    if (!avatar) {
      return session.user.data?.avatar;
    }
  ...
  /**
   * handleEditOrSave handles toggling between edit and save modes for user profile data.
   * In readOnly mode, it enables editing.
   * In edit mode, it saves changes to the server and returns to readOnly mode.
   */
    ...
    <div>
      <div className="shadow-custom mb-8 w-[50%] p-2 rounded-md">
        <div className="flex justify-between">
          ...
          <img
            ...
            alt=""
            className="m-1 w-5 h-5 cursor-pointer"
        ...
        <div className="flex flex-col gap-2 mb-4">
          <div className="flex">
            ...
            <input
              type="text"
              className="bg-transparent p-2 rounded-md shadow-custom focus:outline-none flex-grow"
              placeholder="email"
              readOnly={readOnly}
              value={email}
          ...
          <input
            type="text"
            className="bg-transparent p-2 w-2/5 rounded-md shadow-custom focus:outline-none"
            placeholder="status"
            readOnly={readOnly}
            value={status || ""}
          ...
          <div className="flex flex-row w-full gap-2">
            <textarea
              className="bg-transparent w-2/5 p-2 h-21 rounded-md shadow-custom focus:outline-none resize-none"
              placeholder="Bio"
              readOnly={readOnly}
              value={bio || ""}
        ...
        <button
          className="bg-transparent p-2 rounded-md shadow-custom hover:scale-105"
          onClick={() => logout()}
        >
          Logout
        </button>
      ...
      <div className="shadow-custom mb-8 w-[50%] p-2 rounded-md">
        ...
        <div className="flex flex-col gap-2 mb-4">
          <p
            id="delete_warning"
            className={`${removeWarning === 1 ? "" : "hidden"}`}
          >
            Deleting your account will result in the deletion of all your
            account data, including personal information and preferences. Please
            note that chat messages will remain unaffected. <br /> This action
            cannot be undone.
          </p>
          <p
            id="cannow_delete_message"
            className={`${removeWarning === 2 ? "" : "hidden"}`}
          >
            Were sad to see a member of chirp going. <br /> But u can delete
            your account now with another click at the delete button
          </p>
          <div className="flex justify-center">
            <button
              className="bg-transparent w-5/6 duration-300 ease-in-out transform p-2 rounded-md shadow-custom hover:scale-105"
...
export function NotificationFunc() {
  const [notificationSound, setNotificationSound] = useState<
    string | undefined
  >(session.config.data?.notifications.sound);
  const [muteTime, setMuteTime] = useState<number | undefined>(
    session.config.data?.notifications.mute,
  );
...
export function ChatSettingsFunc() {
  ...
  const [selectedTheme, setSelectedTheme] = useState<string | undefined>(
    session.config.data?.chat.theme,
  );

  const [chatDetails, setChatDetails] = useState<boolean | undefined>(
    session.config.data?.chat.details,
  );
...
export function PrivacyFunc() {
  ...
  const privacyLevels = [
    { displayName: "every one", level: 1 },
    { displayName: "just my friends", level: 2 },
    { displayName: "no one", level: 3 },
  ];
  const [meetInviteLevel, setMeetInviteLevel] = useState<number | undefined>(
    session.config.data?.privacy.meetInviteLevel,
  );
  const [friendInviteLEvel, setFriendInviteLEvel] = useState<
    number | undefined
  >(session.config.data?.privacy.friendInviteLEvel);

  const [blockedUser, setBlockedUser] = useState<blockedUserType[]>([
    { userName: "Currently no blocked user", id: 0 },
  ]);
    ...
    if (userId !== 0) {
      ...
      switch (response.status) {
        case 200:
          if (
            blockedUser[0] &&
            (blockedUser[0].id === 0 || blockedUser[0].id === undefined)
          ) {
    ...
    <div>
      <div className="shadow-custom mb-8 w-[50%] p-2 rounded-md">
        ...
        >
          <input
            type="number"
            className="flex flex-grow bg-transparent focus:outline-none"
          ...
          <img
            ...
            onClick={() => handleBlockUser(userInput || 0)}
          />
        </div>
      ...
      <div className="shadow-custom mb-8 w-[50%] p-2 rounded-md">
        ...
        <div className="flex justify-between mb-2 mr-[50%] p-2 rounded-md shadow-custom">
          ...
          {localStorageSize > 1024 ? (
            <p>{(localStorageSize / 1024).toFixed(1)} kibibytes</p>
          ) : (
            <p>{localStorageSize} bytes</p>
          )}
        </div>
        <div className="flex justify-between mr-[50%] p-2 rounded-md shadow-custom">
          <p>Cookies: </p>
          {cookieSize > 1024 ? (
            <p>{(cookieSize / 1024).toFixed(1)} kibibytes</p>
          ) : (
            <p>{cookieSize} bytes</p>
          )}
        </div>
      </div>
      <div
        id="clearCache"
        className="shadow-custom mb-8 w-[50%] p-2 rounded-md"
      >
        <p className="text-xl mb-2">Clear cache</p>
        <div className={warningMessage ? "" : "hidden"}>
          <p className={`${textaccent} mb-2`}>
            Clearing the cache clears all data stored in your browser associated
            with our CHIRP <br /> service, including that we log you out.
          </p>
          <p className={`${textaccent} font-semibold`}>
            In this process we will clear:
          </p>
        ...
        <button
          className="p-2 rounded-md shadow-custom"
          onClick={() => handleClear(warningMessage)}
        >
...
export function HelpSupportFunc() {
  ...
  return (
    <div onClick={() => setShowForm(false)}>
      <form
        id="form"
        className={`flex flex-col mt-4  w-[50%] ${showForm ? "" : "hidden"}`}
        onSubmit={(e) => handleReport()}
        onClick={(e) => e.stopPropagation()}
      >
        <p className={`text-sm ${textaccent} mb-4`}>
          ! Reports are submitted as an issue by default if something important
          broke please select &quot;Report as a Bug&quot;
        </p>
        <input
          type="text"
          className="bg-transparent p-2 shadow-custom rounded-md focus:outline-none mb-2"
          value={title}
        ...
        <button type="submit" className="p-2 rounded-md shadow-custom">
          Report your {label}
        </button>
      </form>
      <button
        className={`p-2 rounded-md shadow-custom ${showForm ? "hidden" : ""}`}
        onClick={(e) => {
          e.stopPropagation();
          setShowForm(true);
        }}
